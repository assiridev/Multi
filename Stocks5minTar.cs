...
namespace Multi
{
    ...
    class Stocks5minTar : IComparable<Stocks5minTar>
    {
        #region Parameters
        ...
        #endregion
        public Stocks5minTar(string name, DateTime date, double open, double high, double low, double close)
        {
            Name = name; Date = date; Open = open; High = high; Low = low; Close = close;
        }
        public static List<Stocks5minTar> GetStockData(string conn, string symbol, int year, int month)
        {
            ...
            // // Oanda Tbl
            // using (SqlCommand command = new SqlCommand("SELECT TOP 10000000000 * FROM [OandaTbl] WHERE year([_datetime]) >= '" + year + "' and symbol = '" + symbol + "' ORDER BY 3 DESC", connection))
            {
                connection.Open();
                using (SqlDataReader reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        // stockList.Add(new Stocks5minTar(symbol, result, Math.Round(reader.GetDouble(3), 5), Math.Round(reader.GetDouble(4), 5), Math.Round(reader.GetDouble(5), 5), Math.Round(reader.GetDouble(6), 5)));
                        #endregion
                        // ibkr
                        stockList.Add(new Stocks5minTar(symbol, DateTime.Parse(unixTime(reader.GetInt32(2))), Math.Round(reader.GetDouble(3), 5), Math.Round(reader.GetDouble(4), 5), Math.Round(reader.GetDouble(5), 5), Math.Round(reader.GetDouble(6), 5)));
                        // // polygon.io
                        // stockList.Add(new Stocks5minTar(symbol, DateTime.Parse(unixTime(reader.GetDouble(2))), Math.Round(reader.GetDouble(3), 5), Math.Round(reader.GetDouble(4), 5), Math.Round(reader.GetDouble(5), 5), Math.Round(reader.GetDouble(6), 5)));
                    }
                }
            }
            int LastIndex = stockList.Count;
            stockList.Reverse();
            foreach (Stocks5minTar s in stockList)
            {
        ...
        public int CompareTo(Stocks5minTar that)
        {
